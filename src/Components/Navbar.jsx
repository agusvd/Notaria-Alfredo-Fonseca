    ...
    return (
        <div className="bg-black text-white sm:flex sm:justify-around p-5 font-primary">
            <div className="hidden text-center text-xl space-x-2 sm:flex sm:space-x-3 sm:items-center">
                ...
                <Link to="/" className={`hover:no-underline underline-animation cursor-pointer ${
                    location.pathname === '/' ? 'text-teal-500' : 'text-white'
                }`}>Inicio</Link>
                {location.pathname === '/' && ( // Agregar condición para mostrar el menú "Servicios" solo en la página de inicio
                <a href="#servicios" className="hover:no-underline underline-animation cursor-pointer text-white">
                    Servicios
                </a>)}
                <Link to="/Contacto" className={`hover:no-underline underline-animation cursor-pointer ${
                    location.pathname === '/Contacto' ? 'text-teal-500' : 'text-white'
                }`}>Contacto</Link>
                <Link to="/Nosotros" className={`hover:no-underline underline-animation cursor-pointer ${
                    location.pathname === '/Nosotros' ? 'text-teal-500' : 'text-white'
                }`}> Nosotros</Link>
            </div>
